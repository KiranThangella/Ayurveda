...
export default function LoginPage() {
  ...
  const { signInWithPassword, signInWithGoogle, loginAsAdmin } = useAuth();
    ...
    <div className="animate-fade-in min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className={cn('text-2xl font-bold tracking-tight font-display', isTelugu && 'font-telugu')}>{t('auth.login')}</h1>
        ...
        <div className="rounded-2xl border border-border/60 bg-card p-6 sm:p-8 shadow-soft">
          ...
          <div className="space-y-4">
            ...
            >
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <>{t('auth.loginBtn')}<ArrowRight className="h-4 w-4" /></>}
            </button>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-border/40 text-center">
            <button
              ...
              className={cn(
                'w-full flex items-center justify-center gap-2 rounded-lg border border-primary/30 bg-primary/5 py-2 text-xs font-semibold text-primary hover:bg-primary/10 transition-colors',
                isTelugu && 'font-telugu'
              )}
            >
              <Leaf className="h-3.5 w-3.5" />
              {lang === 'te' ? 'అడ్మిన్ డాష్‌బోర్డ్‌లోకి వెళ్లండి' : 'Open Admin Dashboard'}
            </button>
          </div>
        </div>
